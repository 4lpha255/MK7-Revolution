            ...
            switch (choice)
            {
                case 0:
                {
                    while (true)
					{
						...
						std::for_each(items.begin(), items.end(), [&](auto const &i) { options.push_back(std::format("{} ({})", utils::item_name(i).value_or(std::string(magic_enum::enum_name(i))), menu::toggle_name(item_terrain.items.contains(i)))); });
