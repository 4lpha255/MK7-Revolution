...
namespace base
{
    void entries::network::force_replacement_menu(CTRPluginFramework::MenuEntry *entry)
    {
        ...
        while (true)
        {
            ...
            std::for_each(items.begin(), items.end(), [&](auto const &i) { options.push_back(std::format("{} ({})", utils::item_name(i).value_or(std::string(magic_enum::enum_name(i))), menu::s_toggles[force_replacement.items.contains(i)])); });
